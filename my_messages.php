...
$grab_messages = mysql_query("SELECT * FROM message WHERE user_to='$user' && opened='no' && deleted='no'");
...
if ($numrows != 0) {
  while ($get_msg = mysql_fetch_assoc($grab_messages)) {
    ...
    $msg_title = $get_msg['mtitle'];
    $msg_body = $get_msg['mtext'];
    $date = $get_msg['mtime'];
...
if (@$_POST['setopened_' . $id . '']) {
 ...
 $setopened_query = mysql_query("UPDATE message SET opened='yes' WHERE id='$id'");
}
